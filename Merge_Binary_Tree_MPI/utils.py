...
# This class is used to build the binary tree that resemble the dependency graph.
class BuildDependencyGraph:
    def build_dep_tree(self, nprocs):
        tmp = []
        tree_nodes = []
        ...
        while True:
            tmp = []
            mapping = {}
            for idx, node in enumerate(tree_nodes[-1]):
                if idx % 2 == 0:
                    tmp += [node]
                    mapping[node] = node
                else:
                    mapping[node] = tree_nodes[-1][idx-1]

            tree_nodes.append(tmp)
            dep_graph.append(mapping)

            if len(tmp) == 1:
                break

        dep_graph.append({'01': '00'})
